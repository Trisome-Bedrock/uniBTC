...
def test_swapNativeBTCToBTC(fn_isolation, deps, deployer):
    # Upgrade Vault
    ...
    tx = vault.allowTarget([btc_layer2_bridge], {'from': owner})
    assert 'TargetAllowed' in tx.events

    # Swap native BTC to BTC
    native_btc_balance_before = vault.balance()
    amount = bridge_fee * 2
    dest_btc_addr = "1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2"
    tx = m_btc_proxy.swapNativeBTCToBTC(amount, dest_btc_addr, {'from': owner})
    assert 'LockNativeTokenWithBridgeFee' in tx.events   # Emitted by BTCLayer2Bridge.lockNativeToken function
    assert m_btc.balanceOf(vault) == native_btc_balance_before - amount
...
def test_swapMBTCToBTC(fn_isolation, deps, deployer):
    ...
    tx = vault.allowTarget([m_token_swap, btc_layer2_bridge], {'from': owner})
    assert 'TargetAllowed' in tx.events
